...
<div class="bg-white rounded-lg shadow-sm p-4 mb-6">
    <form method="GET" action="/stm/admin/stats/campaigns" class="flex flex-wrap gap-4 items-end">
        <div class="flex-1 min-w-[300px]">
            <label class="block text-sm font-medium text-gray-700 mb-1">Sélectionner une campagne</label>
            <select name="campaign_id" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
    ...
    <div class="flex items-start justify-between">
        <div>
            ...
            <div class="flex items-center gap-4 mt-3 text-sm text-gray-500">
                <span><i class="fas fa-calendar mr-1"></i> <?= date('d/m/Y', strtotime($campaignStats['campaign']['start_date'])) ?> - <?= date('d/m/Y', strtotime($campaignStats['campaign']['end_date'])) ?></span>
                <span><i class="fas fa-globe mr-1"></i> <?= $campaignStats['campaign']['country'] ?></span>
                <span class="px-2 py-1 rounded-full text-xs font-medium <?= $campaignStats['campaign']['status'] === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' ?>">
                    <?= ucfirst($campaignStats['campaign']['status']) ?>
                </span>
            </div>
        </div>
    </div>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
    ...
    <div class="bg-white rounded-lg shadow-sm p-5">
        ...
        <p class="text-2xl font-bold text-gray-900">
            <?= is_numeric($campaignStats['eligible_customers']) ? number_format($campaignStats['eligible_customers'], 0, ',', ' ') : $campaignStats['eligible_customers'] ?>
        </p>
    </div>
    
    <div class="bg-white rounded-lg shadow-sm p-5">
        <p class="text-sm text-gray-500">Clients ayant commandé</p>
        <p class="text-2xl font-bold text-green-600"><?= number_format($campaignStats['customers_ordered'], 0, ',', ' ') ?></p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-5">
        ...
        <p class="text-2xl font-bold text-gray-900"><?= number_format($campaignStats['total_orders'], 0, ',', ' ') ?></p>
    </div>
    
    <div class="bg-white rounded-lg shadow-sm p-5">
        <p class="text-sm text-gray-500">Promos vendues</p>
        <p class="text-2xl font-bold text-orange-600"><?= number_format($campaignStats['total_quantity'], 0, ',', ' ') ?></p>
    </div>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        ...
        <div class="grid grid-cols-3 gap-4 text-center">
            ...
            <div>
                <p class="text-2xl font-bold text-blue-600"><?= number_format($campaignStats['by_country']['BE']['quantity'], 0, ',', ' ') ?></p>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        ...
        <div class="grid grid-cols-3 gap-4 text-center">
            ...
            <div>
                <p class="text-2xl font-bold text-yellow-600"><?= number_format($campaignStats['by_country']['LU']['quantity'], 0, ',', ' ') ?></p>
                <p class="text-xs text-gray-500">Quantités</p>
            </div>
        </div>
    </div>
            ...
            <table class="min-w-full">
                ...
                <tbody class="text-sm">
                    ...
                    <tr class="border-t border-gray-100">
                        ...
                        <td class="py-2 text-right font-bold"><?= number_format($product['quantity_sold'], 0, ',', ' ') ?></td>
            ...
            <table class="min-w-full">
                ...
                <tbody class="text-sm">
                    ...
                    <tr class="border-t border-gray-100">
                        ...
                        <td class="py-2">
                            <span class="px-2 py-1 rounded text-xs <?= $customer['country'] === 'BE' ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800' ?>">
                                <?= $customer['country'] ?>
                            </span>
                        </td>
...
<div class="bg-white rounded-lg shadow-sm p-12 text-center">
    ...
    <p class="text-gray-500">Choisissez une campagne dans la liste ci-dessus pour voir ses statistiques détaillées.</p>
</div>
