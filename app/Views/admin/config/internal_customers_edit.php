<?php
/**
 * Formulaire modification compte interne
 * 
...
// Utiliser les anciennes valeurs si disponibles, sinon les valeurs actuelles
$customerNumber = $old['customer_number'] ?? $customer['customer_number'];
$country = $old['country'] ?? $customer['country'];
$description = $old['description'] ?? $customer['description'];
$isActive = isset($old['is_active']) ? $old['is_active'] : $customer['is_active'];
?>
...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-900">Modifier le compte interne</h1>
</div>

<!-- Formulaire -->
<div class="bg-white rounded-lg shadow">
    <form method="POST" action="/stm/admin/config/internal-customers/<?= $customer['id'] ?>" class="p-6 space-y-6">
        ...
        <div>
            ...
            <input type="text" 
                   ...
                   value="<?= htmlspecialchars($customerNumber) ?>"
        ...
        <div class="bg-gray-50 rounded-lg p-4 text-sm text-gray-600">
            <p><strong>Créé le :</strong> <?= date('d/m/Y à H:i', strtotime($customer['created_at'])) ?></p>
            <?php if ($customer['updated_at'] && $customer['updated_at'] !== $customer['created_at']): ?>
            <p><strong>Dernière modification :</strong> <?= date('d/m/Y à H:i', strtotime($customer['updated_at'])) ?></p>
            <?php endif; ?>
        </div>
...
<div class="mt-6 bg-white rounded-lg shadow border border-red-200">
    <div class="p-6">
        ...
        <p class="text-sm text-gray-600 mb-4">La suppression est définitive et ne peut pas être annulée.</p>
        <form method="POST" action="/stm/admin/config/internal-customers/<?= $customer['id'] ?>/delete" 
              onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce compte interne ?');">
            <input type="hidden" name="_token" value="<?= $_SESSION['csrf_token'] ?>">
            <button type="submit" 
...
$title = 'Modifier compte interne - Configuration';
