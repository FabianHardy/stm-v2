 ...
 * @modified 2025/11/25 13:30 - Correction propriétés nullable
 */
...
class DatabaseSync
{
    ...
    {
        try {
            $this->sourceDb = $this->createConnection($sourceConfig, "source (prod)");
        } catch (Exception $e) {
            $this->connectionError = "Erreur connexion PROD: " . $e->getMessage();
        }

        try {
            $this->targetDb = $this->createConnection($targetConfig, "target (dev)");
        } catch (Exception $e) {
            $this->connectionError =
                ($this->connectionError ? $this->connectionError . " | " : "") .
                "Erreur connexion DEV: " .
                $e->getMessage();
        }
    }
    ...
    {
        return $this->sourceDb !== null && $this->targetDb !== null;
    }
    ...
    public function compareTableStructure(string $table): array
    {
        if ($this->sourceDb === null || $this->targetDb === null) {
            return [["type" => "error", "message" => "Connexions non établies"]];
        }
    ...
    public function verifyAllStructures(array $tables): array
    {
        ...
        if (!$this->isConnected()) {
            ...
            $report["error"] = $this->connectionError;
            return $report;
        }
    ...
    public function syncTable(string $table): array
    {
        ...
        if (!$this->isConnected()) {
            $result["error"] = "Connexions non établies";
            return $result;
        }
        ...
        try {
            ...
            while ($offset < $sourceCount) {
                ...
                $stmt = $this->sourceDb->query("SELECT * FROM `{$table}` LIMIT {$batchSize} OFFSET {$offset}");
